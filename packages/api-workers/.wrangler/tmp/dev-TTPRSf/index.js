...
// .wrangler/tmp/bundle-hS4yRd/checked-fetch.js
...
// .wrangler/tmp/bundle-hS4yRd/strip-cf-connecting-ip-header.js
...
var DatabaseService = class {
  ...
  async getRecipes(filters = {}, pagination = {}) {
    ...
    const recipesQuery = `
      ...
      LIMIT ? OFFSET ?
    `;
    const recipesResult = await this.db.prepare(recipesQuery).bind(...params, limit, skip).all();
  ...
  async getRecipeBakeTypes(recipeId) {
    const query = `
...
var recipes = new Hono2();
recipes.get("/", async (c) => {
  const db = new DatabaseService(c.env.DB);
  const filters = {
    q: c.req.query("q"),
    difficulty: c.req.query("difficulty") ? parseInt(c.req.query("difficulty")) : void 0,
    time: c.req.query("time") ? parseInt(c.req.query("time")) : void 0,
    season: c.req.query("season") ? parseInt(c.req.query("season")) : void 0,
...
// src/routes/bake-types.ts
var bakeTypes = new Hono2();
...
// .wrangler/tmp/bundle-hS4yRd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hS4yRd/middleware-loader.entry.ts
